...
void CDHD_Drive::request_status() {
    g->packSDO(0x600 + id, 0x40, 0x6073, 0, 0);
    g->packSDO(0x600 + id, 0x40, 0x20C7, 0, 0);
}
...
void CDHD_Drive::initialisierung()
{
    ...
    g->packSDO(sdoID, 0x23, 0x1014, 0x00, 0x80 + id); //  0x81  #1014h – COB-ID EMCY

    // 1400h – Receive PDO Communication Parameter 1
    g->packSDO(sdoID, 0x23, 0x1400, 0x01, 0x200 + id); 
    g->packSDO(sdoID, 0x23, 0x1401, 0x01, 0x300 + id); 
    g->packSDO(sdoID, 0x23, 0x1402, 0x01, 0x400 + id); 
    g->packSDO(sdoID, 0x23, 0x1403, 0x01, 0x500 + id); 
    ...
    g->packSDO(sdoID, 0x23, 0x1800, 0x01, 0x180 + id); 
    g->packSDO(sdoID, 0x23, 0x1801, 0x01, 0x280 + id); 
    g->packSDO(sdoID, 0x23, 0x1802, 0x01, 0x380 + id); 
    g->packSDO(sdoID, 0x23, 0x1803, 0x01, 0x480 + id); 
