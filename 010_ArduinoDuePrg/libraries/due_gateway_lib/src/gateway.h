...
public:
    ...
    uint32_t sendSDO(uint32_t s_id, uint8_t s_cb, uint16_t s_od, uint8_t s_sub, uint32_t s_data);
    bool checkSDO(CAN_FRAME &frame, uint16_t s_od, uint8_t s_sub, uint32_t* data);
    
    bool changeMode(CDHD_Drive& servo, int8_t mode);
    bool changeState(CDHD_Drive& servo, uint16_t control, uint16_t wantedState);
    void set_nmt_state(uint16_t state, uint32_t _id)
    {
        init_Msg.data.byte[0] = state;
        init_Msg.data.byte[1] = _id;
